...
bool IsPhysicalConsoleSession() {
    ...
    if (buffer && bytes >= sizeof(USHORT))
        console = (*reinterpret_cast<USHORT*>(buffer) == 0);
    if (buffer)
...
HRESULT RetrieveNegotiateAuthPackage(ULONG* packageId) {
    if (!packageId)
        return E_INVALIDARG;
    *packageId = 0;
    LSA_HANDLE h = nullptr;
    NTSTATUS st = LsaConnectUntrusted(&h);
    if (st < 0)
        return HRESULT_FROM_WIN32(LsaNtStatusToWinError(st));
    LSA_STRING name{};
    const char kName[] = "Negotiate";
    name.Buffer = const_cast<PCHAR>(kName);
    name.Length = sizeof(kName) - 1;
    name.MaximumLength = sizeof(kName);
    st = LsaLookupAuthenticationPackage(h, &name, packageId);
...
std::wstring RunFaceSensor(void* moduleHandle, int timeoutMs) {
    ...
    BOOL ok = CreateProcessW(exe.c_str(), mutableCmd.data(), nullptr, nullptr, TRUE,
    ...
    const ULONGLONG deadline =
        GetTickCount64() + static_cast<ULONGLONG>(timeoutMs + kSensorStartupGraceMs);
    std::string out;
    bool timedOut = false;
    for (;;) {
        DWORD available = 0;
        if (PeekNamedPipe(readPipe, nullptr, 0, nullptr, &available, nullptr) && available) {
            ...
            if (ReadFile(readPipe, buf, want, &got, nullptr) && got)
