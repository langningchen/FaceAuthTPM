...
static const CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR kFields[FID_COUNT]={
    // Deliberately omit CPFT_TILE_IMAGE. LogonUI supplies its native default
    // provider image, avoiding the black/white rectangle produced when it
    // ignores alpha in a third-party HBITMAP.
...
void AppendProviderLog(const wchar_t* stage){
    ...
    wchar_t wide[320]{};
    int chars=swprintf_s(wide,L"[%04u-%02u-%02uT%02u:%02u:%02u.%03uZ] tick=%llu pid=%lu %ls\r\n",
...
void AppendCredentialCountLog(size_t count,const wchar_t* scope){
    wchar_t message[128]{};
    swprintf_s(message,L"credentials_built count=%zu scope=%ls",count,scope?scope:L"unknown");
    AppendProviderLog(message);
}

HRESULT CopyField(const CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR& in,CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR** out){
    if(!out)return E_INVALIDARG; *out=nullptr;
    auto* p = static_cast<CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR*>(
        CoTaskMemAlloc(sizeof(CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR)));
    if (!p) return E_OUTOFMEMORY;
    *p=in; p->pszLabel=nullptr;
...
FaceProvider::~FaceProvider(){UnAdvise();ClearCredentials();DllRelease();}

HRESULT FaceProvider::QueryInterface(REFIID riid,void** ppv){
    if(!ppv)return E_INVALIDARG;
    *ppv=nullptr;
    if(riid==IID_IUnknown||riid==IID_ICredentialProvider)*ppv=static_cast<ICredentialProvider*>(this);
    else return E_NOINTERFACE;
    AddRef();
    return S_OK;
}
ULONG FaceProvider::AddRef(){return ++ref_;}
ULONG FaceProvider::Release(){ULONG r=--ref_;if(!r)delete this;return r;}

HRESULT FaceProvider::SetUsageScenario(CREDENTIAL_PROVIDER_USAGE_SCENARIO cpus,DWORD flags){
    cpus_=cpus;flags_=flags;
    if(cpus!=CPUS_LOGON&&cpus!=CPUS_UNLOCK_WORKSTATION)return E_NOTIMPL;
    if(!faceauth::IsFaceAuthSessionAllowed())return E_NOTIMPL;
...
HRESULT FaceProvider::SetSerialization(const CREDENTIAL_PROVIDER_CREDENTIAL_SERIALIZATION*){return E_NOTIMPL;}
...
HRESULT FaceProvider::Advise(ICredentialProviderEvents* e,UINT_PTR c){
    ...
    if(matched && e)e->CredentialsChanged(c);
    else MaybeStartRecognition();
    return S_OK;
}
...
HRESULT FaceProvider::GetFieldDescriptorCount(DWORD* c){if(!c)return E_INVALIDARG;*c=FID_COUNT;return S_OK;}
HRESULT FaceProvider::GetFieldDescriptorAt(DWORD i,CREDENTIAL_PROVIDER_FIELD_DESCRIPTOR** f){if(i>=FID_COUNT)return E_INVALIDARG;return CopyField(kFields[i],f);}

HRESULT FaceProvider::GetCredentialCount(DWORD* count,DWORD* def,BOOL* autoLogon){
    if(!count||!def||!autoLogon)return E_INVALIDARG;
    std::scoped_lock lock(credentialMutex_);
    *count=static_cast<DWORD>(credentials_.size());
    *def=CREDENTIAL_PROVIDER_NO_DEFAULT;
    *autoLogon=FALSE;
    if(autoArmed_.exchange(false) && !matchedSid_.empty() && !faceauth::IsInCooldown(matchedSid_)){
        for(size_t i=0;i<credentials_.size();++i){
            if(credentials_[i]->Sid()==matchedSid_){*def=static_cast<DWORD>(i);*autoLogon=TRUE;break;}
        }
    }
    return S_OK;
}
...
HRESULT FaceProvider::GetCredentialAt(DWORD i,ICredentialProviderCredential** c){
    ...
    std::scoped_lock lock(credentialMutex_);
    if(i>=credentials_.size())return E_INVALIDARG;
    *c=credentials_[i];(*c)->AddRef();return S_OK;
}
...
void FaceProvider::RebuildCredentials(){
    ...
    if(cpus_==CPUS_UNLOCK_WORKSTATION) unlockSid=faceauth::ActiveConsoleUserSidString();

    for(const auto& identity:identities){
        if(identity.sid.empty()||identity.qualifiedName.empty()||!identity.onlineIdentity)continue;
        if(!faceauth::CredentialExists(identity.sid))continue;
        std::error_code ec;
        if(!std::filesystem::is_regular_file(faceauth::ProfilePath(identity.sid),ec))continue;
        if(cpus_==CPUS_UNLOCK_WORKSTATION){
            if(unlockSid.empty() || identity.sid!=unlockSid)continue;
        }
        auto* credential=new(std::nothrow) FaceCredential(cpus_,identity.sid,identity.qualifiedName,identity.displayName);
        if(credential)fresh.push_back(credential);
    }
    ...
    AppendCredentialCountLog(count,cpus_==CPUS_UNLOCK_WORKSTATION?L"unlock-current-user":L"logon-enrolled-users");
}

bool FaceProvider::HasCredentialForSid(const std::wstring& sid){
    std::scoped_lock lock(credentialMutex_);
    for(auto* c:credentials_)if(c->Sid()==sid)return true;
    return false;
}

void FaceProvider::MaybeStartRecognition(){
    if((cpus_!=CPUS_LOGON&&cpus_!=CPUS_UNLOCK_WORKSTATION)||!faceauth::IsFaceAuthSessionAllowed())return;
    {
        std::scoped_lock lock(credentialMutex_);
        if(credentials_.empty())return;
        if(autoArmed_.load()&&!matchedSid_.empty())return;
    }
    bool expected=false;
    if(!recognitionStarted_.compare_exchange_strong(expected,true))return;
    ...
    if(h)CloseHandle(h);else{recognitionStarted_=false;Release();}
}
...
void FaceProvider::RecognitionWorker(){
    for(;;){
        if((cpus_!=CPUS_LOGON&&cpus_!=CPUS_UNLOCK_WORKSTATION)||!faceauth::IsFaceAuthSessionAllowed())break;
        ...
        if(sid.empty()){
            bool advised=false;
            {std::scoped_lock lock(eventMutex_);advised=(events_!=nullptr);}
            if(!advised)break;
            Sleep(1500);
            continue;
        }
        ...
        if(faceauth::IsInCooldown(sid))break;
        ...
        ICredentialProviderEvents* ev=nullptr;UINT_PTR ctx=0;
        {
            std::scoped_lock lock(eventMutex_);
            ev=events_;ctx=adviseContext_;if(ev)ev->AddRef();
        }
        if(ev){ev->CredentialsChanged(ctx);ev->Release();}
        break;
    }
