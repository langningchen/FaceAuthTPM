...
void AppendStartupLog(const Clock::time_point& start, const wchar_t* stage,
    ...
    std::wstring line =
        L"[" + std::to_wstring(st.wYear) + L"-" + (st.wMonth < 10 ? L"0" : L"") +
        std::to_wstring(st.wMonth) + L"-" + (st.wDay < 10 ? L"0" : L"") + std::to_wstring(st.wDay) +
        L"T" + (st.wHour < 10 ? L"0" : L"") + std::to_wstring(st.wHour) + L":" +
...
std::wstring ReadPreferredBackend() {
    ...
    value = Lower(value);
    if (value != L"msmf" && value != L"dshow" && value != L"any")
        return L"msmf";
    return value;
}
...
bool OpenCameraFast(cv::VideoCapture& camera, int cameraIndex, const Clock::time_point& started,
    ...
    for (const auto& backend : BackendOrder(preferred)) {
        ...
        if (!ok)
            ok = camera.open(cameraIndex, backend.api);
        const auto openMs =
        ...
        if (ok) {
            ...
            AppendStartupLog(started, L"camera_open_success",
                             L"backend=" + selectedBackend + L" open_ms=" +
                                 std::to_wstring(openMs) + L" format=" + std::to_wstring(w) + L"x" +
                                 std::to_wstring(h) + L" fps=" + std::to_wstring(fps));
            return true;
        }
        AppendStartupLog(started, L"camera_open_failed_backend",
                         L"backend=" + std::wstring(backend.token) + L" open_ms=" +
                             std::to_wstring(openMs));
    }
    if (error)
...
int wmain(int argc, wchar_t** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::wstring(argv[i]) == L"--timeout-ms" && i + 1 < argc)
            timeout = _wtoi(argv[++i]);
        else if (std::wstring(argv[i]) == L"--camera" && i + 1 < argc)
    ...
    while (Clock::now() - firstBegin < std::chrono::milliseconds(1500)) {
        if (camera.read(firstFrame) && !firstFrame.empty()) {
            gotFirst = true;
            break;
        }
        Sleep(10);
    ...
    const auto firstMs =
        std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() - firstBegin).count();
    AppendStartupLog(started, gotFirst ? L"camera_first_frame" : L"camera_first_frame_timeout",
                     L"backend=" + selectedBackend + L" first_frame_ms=" +
                         std::to_wstring(firstMs) +
                         (gotFirst ? L" format=" + std::to_wstring(firstFrame.cols) + L"x" +
                                         std::to_wstring(firstFrame.rows)
                                   : L""));
    ...
    DWORD written = 0;
    WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), line.data(), static_cast<DWORD>(line.size()),
              &written, nullptr);
    return 0;
}
