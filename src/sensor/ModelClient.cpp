...
bool ScanUsingModelService(cv::VideoCapture& camera, int timeoutMs, std::wstring& sid, std::wstring* error, const cv::Mat* firstFrame) {
    sid.clear();
    if (!camera.isOpened()) {
        if (error) *error = L"camera is not open";
        return false;
    }

    HANDLE pipe = ConnectModelPipe();
    if (pipe == INVALID_HANDLE_VALUE) {
        if (error) *error = L"preloaded model service is not available (Win32 " + std::to_wstring(GetLastError()) + L")";
        return false;
    }
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if (useFirstFrame) {
            frame = *firstFrame;
            useFirstFrame = false;
        } else if (!camera.read(frame)) { Sleep(20); continue; }
        if (frame.empty()) continue;
        if (frame.type() != CV_8UC3) {
            if (error) *error = L"unexpected camera frame format";
            CloseHandle(pipe);
            return false;
        }
        ...
        if (bytes64 == 0 || bytes64 > kModelMaxFrameBytes) {
            if (error) *error = L"camera frame is too large";
            CloseHandle(pipe);
            return false;
        }

        ModelFrameHeader header{
            kModelPipeMagic,
            kModelPipeVersion,
        ...
        if (!WriteExact(pipe, &header, sizeof(header)) ||
            ...
            if (error) *error = L"model service pipe write failed";
            CloseHandle(pipe);
            return false;
        }
        ...
        if (!ReadExact(pipe, &response, sizeof(response)) ||
            response.magic != kModelPipeMagic || response.version != kModelPipeVersion ||
            response.sidChars > 256) {
            if (error) *error = L"model service returned an invalid response";
            CloseHandle(pipe);
            return false;
        }
        ...
        if (response.sidChars) {
            ...
            if (!ReadExact(pipe, candidate.data(), sidBytes)) {
                if (error) *error = L"model service SID read failed";
                CloseHandle(pipe);
                return false;
            }
        }
        ...
        if (response.status == kModelStatusError) {
            if (error) *error = L"model service reported an inference error";
            CloseHandle(pipe);
            return false;
        }
        if (response.status == kModelStatusMatch && !candidate.empty()) {
            if (candidate == lastSid) ++consecutive;
            else { lastSid = std::move(candidate); consecutive = 1; }
