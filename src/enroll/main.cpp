...
void Usage() {
    std::wcout << L"FaceAuthEnroll commands:\n"
                  ...
                  L"      Capture face, save TPM-encrypted Windows password, and cache identity "
                  L"metadata.\n"
                  ...
                  L"  test [--camera N]     Verify face/profile/vault/identity metadata (does not "
                  L"decrypt password).\n"
...
int GetCamera(int argc, wchar_t** argv) {
    for (int i = 2; i + 1 < argc; ++i)
        if (std::wstring(argv[i]) == L"--camera")
            return _wtoi(argv[i + 1]);
    return 0;
}

bool PrepareIdentity(int argc, wchar_t** argv, faceauth::IdentityRecord& identity,
                     std::wstring& error) {
    const std::wstring qualified = GetOption(argc, argv, L"--qualified");
    const std::wstring display = GetOption(argc, argv, L"--display");
    if (!faceauth::BuildCurrentIdentityRecord(identity, qualified, display, &error))
        return false;
    return true;
}
...
void PrintIdentity(const faceauth::IdentityRecord& identity) {
    ...
    if (!identity.onlineIdentity) {
        std::wcout << L"Note: FaceAuth 0.5 production auto-logon currently enumerates "
                      L"online/Microsoft identities only.\n";
    }
}
} // namespace
...
int wmain(int argc, wchar_t** argv) {
    ...
    if (sid.empty()) {
        std::wcerr << L"Could not determine current user's SID.\n";
        return 4;
    }
    ...
    if (cmd == L"identity") {
        faceauth::IdentityRecord identity;
        std::wstring error;
        if (!PrepareIdentity(argc, argv, identity, error)) {
            std::wcerr << L"Identity detection failed: " << error << L"\n"
                       << L"For a Microsoft account, retry with --qualified "
                          L"\"MicrosoftAccount\\you@example.com\".\n";
            return 14;
        }
    ...
    if (cmd == L"test") {
        ...
        if (!engine.Initialize(&error)) {
            std::wcerr << L"Face engine error: " << error << L"\n";
            return 6;
        }
        faceauth::FaceProfile p;
        if (!faceauth::LoadFaceProfile(sid, p, &error)) {
            std::wcerr << L"Profile: " << error << L"\n";
            return 7;
        }
        if (!faceauth::CredentialExists(sid)) {
            std::wcerr << L"Encrypted credential blob is missing.\n";
            return 8;
        }
        std::wcout << L"Encrypted credential blob is present. The TPM private key is SYSTEM-only "
                      L"by design;\n"
                      L"run scripts\\test-vault-as-system.ps1 to verify decryption without "
                      L"exposing the password.\n"
    ...
    std::wcout << L"Enter the Windows PASSWORD used by Windows for this account.\n"
                  L"For Microsoft accounts this is the Microsoft account password; do not enter a "
                  L"Windows Hello PIN.\n"
                  L"Input is hidden: ";
    std::wstring password = ReadPasswordNoEcho();
    if (password.empty()) {
        std::wcerr << L"Empty password; enrollment cancelled.\n";
        return 11;
    }
    ...
    faceauth::ClearCooldown(sid);
    std::wcout << L"Enrollment complete. Run 'FaceAuthEnroll test', then "
                  L"scripts\\test-vault-as-system.ps1, before enabling the Credential Provider.\n";
    return 0;
}
