...
int Finish(int code, const std::wstring& message, const std::wstring& resultFile, bool error){
    if(error) std::wcerr << message << L"\n";
    else std::wcout << message << L"\n";
...
int wmain(int argc, wchar_t** argv){
    ...
    for(int i=1;i<argc;++i){
        ...
        if(arg==L"--bootstrap") bootstrap=true;
        else if(arg==L"--sid" && i+1<argc) sid=argv[++i];
        else if(arg==L"--result-file" && i+1<argc) resultFile=argv[++i];
    }
    if(!faceauth::IsRunningAsLocalSystem()){
        return Finish(5, L"This probe must run as LocalSystem. Use the provided PowerShell scripts.", resultFile, true);
    }
    if(bootstrap){
        std::wstring error;
        ...
        return Finish(0, L"TPM vault bootstrap OK. Private key is machine-bound, non-exportable, and SYSTEM-only.", resultFile, false);
    }
    if(sid.empty()){
        Usage();
    ...
    return Finish(0, L"TPM vault decrypt OK for SID " + sid + L". Plaintext was not displayed.", resultFile, false);
}
