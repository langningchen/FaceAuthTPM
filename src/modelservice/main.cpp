...
void AppendLog(const wchar_t* stage, const std::wstring& detail = {}) {
    ...
    std::wstring line = L"[" + std::to_wstring(st.wYear) + L"-" +
        ...
        (st.wSecond < 10 ? L"0" : L"") + std::to_wstring(st.wSecond) + L"Z] " + stage;
...
bool ReadExact(HANDLE h, void* buffer, DWORD bytes) {
    ...
    while (done < bytes) {
        DWORD got = 0;
        if (!ReadFile(h, p + done, bytes - done, &got, nullptr) || got == 0) return false;
        done += got;
    }
    return true;
}
...
bool WriteExact(HANDLE h, const void* buffer, DWORD bytes) {
    ...
    while (done < bytes) {
        DWORD wrote = 0;
        if (!WriteFile(h, p + done, bytes - done, &wrote, nullptr) || wrote == 0) return false;
        done += wrote;
    }
    return true;
}
...
bool SendResponse(HANDLE pipe, std::uint32_t status, const std::wstring& sid = {}) {
    faceauth::ModelResponseHeader response{
        faceauth::kModelPipeMagic,
        faceauth::kModelPipeVersion,
        status,
        static_cast<std::uint32_t>(sid.size())
    };
    if (!WriteExact(pipe, &response, sizeof(response))) return false;
    if (!sid.empty()) {
        const DWORD bytes = static_cast<DWORD>(sid.size() * sizeof(wchar_t));
        if (!WriteExact(pipe, sid.data(), bytes)) return false;
    }
    return true;
}

std::wstring MatchFrame(faceauth::FaceEngine& engine,
                        const std::vector<faceauth::FaceProfile>& profiles,
                        const cv::Mat& frame) {
    ...
    if (!engine.ExtractEmbedding(inference, embedding, nullptr)) return {};
    ...
    for (const auto& p : profiles) {
        ...
        if (score > best) { second = best; best = score; winner = &p; }
        else if (score > second) second = score;
    }
    if (!winner || best < faceauth::kMatchThreshold) return {};
    if (profiles.size() > 1 && best - second < faceauth::kMatchMargin) return {};
    return winner->sid;
}
...
void HandleClient(HANDLE pipe, faceauth::FaceEngine& engine) {
    ...
    for (;;) {
        ...
        if (header.magic != faceauth::kModelPipeMagic ||
            header.version != faceauth::kModelPipeVersion ||
            header.channels != 3 || header.width == 0 || header.height == 0 ||
            header.stride != header.width * header.channels ||
            header.dataBytes != header.stride * header.height ||
            header.width > 3840 || header.height > 2160 ||
            header.dataBytes > faceauth::kModelMaxFrameBytes) {
            SendResponse(pipe, faceauth::kModelStatusError);
            return;
        }

        std::vector<unsigned char> frameBytes(header.dataBytes);
        if (!ReadExact(pipe, frameBytes.data(), header.dataBytes)) return;
        cv::Mat frame(static_cast<int>(header.height), static_cast<int>(header.width),
                      CV_8UC3, frameBytes.data(), static_cast<size_t>(header.stride));
        const auto sid = MatchFrame(engine, profiles, frame);
        if (!SendResponse(pipe, sid.empty() ? faceauth::kModelStatusNoMatch : faceauth::kModelStatusMatch, sid)) return;
...
bool RunServer(faceauth::FaceEngine& engine) {
    ...
    while (WaitForSingleObject(g_stopEvent, 0) != WAIT_OBJECT_0) {
        ...
        if (!ov.hEvent) { CloseHandle(pipe); LocalFree(descriptor); return false; }

        BOOL connected = ConnectNamedPipe(pipe, &ov);
        DWORD error = connected ? ERROR_SUCCESS : GetLastError();
        if (!connected && error == ERROR_PIPE_CONNECTED) SetEvent(ov.hEvent);
        else if (!connected && error != ERROR_IO_PENDING) {
            CloseHandle(ov.hEvent); CloseHandle(pipe); continue;
        }
...
void WINAPI ServiceMain(DWORD, LPWSTR*) {
    ...
    if (!g_stopEvent) { SetServiceState(SERVICE_STOPPED, GetLastError()); return; }
...
int wmain(int argc, wchar_t** argv) {
    ...
    SERVICE_TABLE_ENTRYW table[] = {
        {const_cast<LPWSTR>(faceauth::kModelServiceName), ServiceMain},
        {nullptr, nullptr}
    };
