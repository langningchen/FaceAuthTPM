...
bool LookupSidName(const std::wstring& sidString, std::wstring& qualified, std::wstring& display) {
    ...
    if (GetLastError() != ERROR_INSUFFICIENT_BUFFER || !nameChars) {
        LocalFree(sid);
        return false;
    }
    std::vector<wchar_t> name(nameChars), domain(domainChars ? domainChars : 1);
    if (!LookupAccountSidW(nullptr, sid, name.data(), &nameChars, domain.data(), &domainChars,
                           &use)) {
        LocalFree(sid);
        return false;
    }
    ...
    if (domainChars && domain[0])
        qualified = std::wstring(domain.data()) + L"\\" + name.data();
    else
        qualified.assign(name.data());
    return true;
}

bool CurrentLogonSessionStrings(std::wstring& user, std::wstring& domain, std::wstring& upn,
                                std::wstring& authPackage) {
    ...
    if (LsaGetLogonSessionData(&stats.AuthenticationId, &data) != 0 || !data)
        return false;
...
bool SaveIdentityRecord(const IdentityRecord& identity, std::wstring* error) {
    if (identity.sid.empty() || identity.qualifiedName.empty() || identity.sid.size() > 256 ||
        ...
        if (error)
            *error = L"Invalid identity metadata";
        return false;
    }
    ...
    Header h{kMagic,
             kVersion,
             identity.onlineIdentity ? kFlagOnlineIdentity : 0,
    ...
    auto path = IdentityPath(identity.sid), tmp = path;
    tmp += L".tmp";
    std::ofstream f(tmp, std::ios::binary | std::ios::trunc);
    if (!f) {
        if (error)
            *error = L"Cannot create identity metadata";
        return false;
    }
    f.write(reinterpret_cast<const char*>(&h), sizeof(h));
    f.write(reinterpret_cast<const char*>(identity.sid.data()),
            identity.sid.size() * sizeof(wchar_t));
    f.write(reinterpret_cast<const char*>(identity.qualifiedName.data()),
            identity.qualifiedName.size() * sizeof(wchar_t));
    f.write(reinterpret_cast<const char*>(identity.displayName.data()),
            identity.displayName.size() * sizeof(wchar_t));
    f.close();
    if (!f || !ApplyAdminSystemOnlyAcl(tmp)) {
        std::filesystem::remove(tmp);
        if (error)
            *error = L"Writing/securing identity metadata failed";
        return false;
    }
    std::error_code ec;
    ...
    if (ec) {
        if (error)
            *error = L"Replacing identity metadata failed";
        return false;
    }
...
bool LoadIdentityRecord(const std::wstring& sid, IdentityRecord& identity, std::wstring* error) {
    ...
    if (!f) {
        if (error)
            *error = L"Identity metadata not found";
        return false;
    }
    Header h{};
    f.read(reinterpret_cast<char*>(&h), sizeof(h));
    if (!f || h.magic != kMagic || h.version != kVersion || h.sidChars > 256 ||
        h.qualifiedChars == 0 || h.qualifiedChars > 1024 || h.displayChars > 1024) {
        if (error)
            *error = L"Invalid identity metadata header";
        return false;
    }
    std::wstring fileSid(h.sidChars, L'\0'), qualified(h.qualifiedChars, L'\0'),
        display(h.displayChars, L'\0');
    ...
    if (!display.empty())
        f.read(reinterpret_cast<char*>(display.data()), display.size() * sizeof(wchar_t));
    if (!f || fileSid != sid) {
        if (error)
            *error = L"Identity metadata SID mismatch/truncated";
        return false;
    }
    identity = {std::move(fileSid), std::move(qualified), std::move(display),
                (h.flags & kFlagOnlineIdentity) != 0};
    if (identity.displayName.empty())
        identity.displayName = identity.qualifiedName;
    return true;
}
...
std::vector<IdentityRecord> LoadAllIdentityRecords() {
    ...
    std::error_code ec;
    if (!std::filesystem::exists(IdentitiesDir(), ec))
        return out;
    for (const auto& e : std::filesystem::directory_iterator(IdentitiesDir(), ec)) {
        if (ec)
            break;
        if (!e.is_regular_file() || e.path().extension() != L".fai")
            continue;
        IdentityRecord r;
        if (LoadIdentityRecord(e.path().stem().wstring(), r, nullptr))
...
bool DeleteIdentityRecord(const std::wstring& sid) {
    std::error_code ec;
    return !std::filesystem::exists(IdentityPath(sid), ec) ||
           std::filesystem::remove(IdentityPath(sid), ec);
}
bool IdentityRecordExists(const std::wstring& sid) {
    std::error_code ec;
    return std::filesystem::is_regular_file(IdentityPath(sid), ec);
}
...
bool BuildCurrentIdentityRecord(IdentityRecord& identity, const std::wstring& qualifiedOverride,
    ...
    if (identity.sid.empty()) {
        if (error)
            *error = L"Could not determine current user SID";
        return false;
    }
    ...
    if (!qualifiedOverride.empty()) {
        ...
        if (!account.empty())
            identity.qualifiedName = domain + L"\\" + account;
    ...
    if (identity.qualifiedName.empty())
        identity.qualifiedName = lookupQualified;
    if (!qualifiedOverride.empty() && IsLikelyOnlineQualifiedName(qualifiedOverride))
        identity.onlineIdentity = true;
    if (identity.qualifiedName.empty()) {
        if (error)
            *error = L"Could not determine the qualified Windows account name";
        return false;
    }

    if (!displayOverride.empty())
        identity.displayName = displayOverride;
    else if (identity.onlineIdentity) {
        const auto slash = identity.qualifiedName.find(L'\\');
        identity.displayName = (slash == std::wstring::npos)
                                   ? identity.qualifiedName
                                   : identity.qualifiedName.substr(slash + 1);
    } else if (!lookupDisplay.empty())
        identity.displayName = lookupDisplay;
    else
        identity.displayName = identity.qualifiedName;
