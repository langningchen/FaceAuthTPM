...
bool ApplyAdminSystemOnlyAcl(const std::filesystem::path& path) {
    ...
    if (ok) {
        DWORD e = SetNamedSecurityInfoW(const_cast<LPWSTR>(path.c_str()), SE_FILE_OBJECT,
...
bool IsRunningAsLocalSystem() {
    ...
    if (!cb) { CloseHandle(token); return false; }
    std::vector<unsigned char> buf(cb);
    if (!GetTokenInformation(token, TokenUser, buf.data(), cb, &cb)) { CloseHandle(token); return false; }
    CloseHandle(token);
...
std::wstring CurrentUserSidString() {
    ...
    GetTokenInformation(token, TokenUser, nullptr, 0, &cb);
    std::vector<unsigned char> buf(cb);
    if (!GetTokenInformation(token, TokenUser, buf.data(), cb, &cb)) { CloseHandle(token); return {}; }
    auto* tu = reinterpret_cast<TOKEN_USER*>(buf.data());
    LPWSTR sid = nullptr;
    std::wstring out;
    if (ConvertSidToStringSidW(tu->User.Sid, &sid)) { out = sid; LocalFree(sid); }
    CloseHandle(token);
    return out;
}
...
std::wstring ActiveConsoleUserSidString() {
    ...
    if (WTSQueryUserToken(sessionId, &token)) {
        ...
        if (cb) {
            ...
            if (GetTokenInformation(token, TokenUser, buf.data(), cb, &cb)) {
                ...
                if (!out.empty()) { CloseHandle(token); return out; }
            }
        }
        CloseHandle(token);
    }
    ...
    if (!WTSQuerySessionInformationW(WTS_CURRENT_SERVER_HANDLE, sessionId, WTSUserName, &user, &userBytes) || !user || !*user) {
        if (user) WTSFreeMemory(user);
        return {};
    }
    WTSQuerySessionInformationW(WTS_CURRENT_SERVER_HANDLE, sessionId, WTSDomainName, &domain, &domainBytes);
    std::wstring qualified = (domain && *domain) ? (std::wstring(domain) + L"\\" + user) : std::wstring(user);
    WTSFreeMemory(user);
    ...
    if (GetLastError() != ERROR_INSUFFICIENT_BUFFER || !sidBytes) return {};
    std::vector<unsigned char> sid(sidBytes);
    std::vector<wchar_t> resolvedDomain(nameBytes ? nameBytes : 1);
    if (!LookupAccountNameW(nullptr, qualified.c_str(), sid.data(), &sidBytes, resolvedDomain.data(), &nameBytes, &use)) return {};
    LPWSTR sidText = nullptr;
    std::wstring out;
    if (ConvertSidToStringSidW(sid.data(), &sidText)) { out = sidText; LocalFree(sidText); }
    return out;
}
...
void SecureErase(std::vector<wchar_t>& v) {
    if (!v.empty()) SecureZeroMemory(v.data(), v.size() * sizeof(wchar_t));
    v.clear(); v.shrink_to_fit();
}
}
