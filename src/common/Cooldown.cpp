...
void SetCooldown(const std::wstring& sid, unsigned seconds) {
    ...
    std::ofstream f(CooldownPath(sid), std::ios::trunc);
    if (f) {
        f << (NowSeconds() + seconds);
        f.close();
        ApplyAdminSystemOnlyAcl(CooldownPath(sid));
    }
}
bool IsInCooldown(const std::wstring& sid) {
    std::ifstream f(CooldownPath(sid));
    long long until = 0;
    if (!(f >> until))
        return false;
    if (until <= NowSeconds()) {
...
void ClearCooldown(const std::wstring& sid) {
    std::error_code ec;
    std::filesystem::remove(CooldownPath(sid), ec);
}
} // namespace faceauth
